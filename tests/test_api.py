...
from usgsm2m import api, errors, util
...
def test_api_login(usgs_api):
    assert usgs_api.session.headers.get("X-Auth-Token")
...
def test_api_get_scene_id(usgs_api):

    # Single Product ID
    PRODUCT_ID = "LT05_L1GS_173058_20111028_20161005_01_T2"
    scene_id = usgs_api.get_entity_id(PRODUCT_ID, dataset="landsat_tm_c1")
    ...
    scene_ids = usgs_api.get_entity_id(PRODUCT_IDS, dataset="landsat_tm_c1")
    assert scene_ids == ["LT51730582011301MLK00", "LT51730572001097LBG00"]


def test_api_metadata(usgs_api):
    ...
    for display_id in PRODUCTS:
        ...
        entity_id = usgs_api.get_entity_id(display_id, dataset)
        metadata = usgs_api.metadata(entity_id, dataset)
...
def test_api_get_product_id(usgs_api):

    SCENE_ID = "LT51730582011301MLK00"

    # Collection 1
    product_id = usgs_api.get_display_id(SCENE_ID, "landsat_tm_c1")
    assert product_id == "LT05_L1GS_173058_20111028_20161005_01_T2"

    # Collection 2
    product_id = usgs_api.get_display_id(SCENE_ID, "landsat_tm_c2_l2")
    assert product_id == "LT05_L2SP_173058_20111028_20200820_02_T1"


def test_api_search(usgs_api):
